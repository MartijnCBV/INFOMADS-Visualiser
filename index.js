...
function submitProblem() {
    ...
    const str = document.getElementById("problem-in").value;
    const problem = parseProblem(str);
    const svg = problemSVG(problem, true);
    if (out)
        out.innerHTML = makeSVG([svg[0]], svg[1], svg[2]);
}
function submitSolution() {
    const out = document.getElementById("canvas");
    const pstr = document.getElementById("problem-in").value;
    const sstr = document.getElementById("solution-in").value;
    ...
    if (out)
        out.innerHTML = makeSVG([ssvg[0], psvg[0], ssvg[1]], psvg[1], psvg[2]);
}
function parseProblem(str) {
    const lines = str.split("\n");
    const handleLine = (line) => line.replace(/\s/g, "").split(",").map(j => parseInt(j));
    const students = [];
...
function parseSolution(str, problem) {
    const lines = str.split("\n");
    const handleLine = (line) => line.replace(/\s/g, "").split(",").map(j => parseInt(j));
    const obligations = [];
    let i = 1;
...
function makeSVG(strs, height, width) {
...
function solutionSVG(solution, problem, colorState) {
    let i = 1;
